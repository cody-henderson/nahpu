...
class MergeTestStruct {
  ...
  Future<void> oneTimeSetup() async {
    ...
    final sitesStr = await getSites(dataSet);

    sites = (jsonDecode(sitesStr) as List)
  ...
  Future<void> testSetup() async {
    ...
    dbSites = [];
    for (ProjectData project in dbProjects) {
      dbSites.addAll(await SiteQuery(db).getAllSites(project.uuid));
    }
  }
  ...
  Future<void> validateDB() async {
    expect(UnorderedIterableEquality().equals(dbProjects, projects), true);
    expect(UnorderedIterableEquality().equals(dbSites, sites), true);
  }
}
...
void main() {
  ...
  test('validate initial database structure', () async {
    await structA.validateDB();
    await structB.validateDB();
  });
  ...
  test('merge DBs, keep all projects', () async {
    ...
    final combinedSites = structA.sites + structB.sites;
    ...
    // Site checks
    // List should match except for ids
    expect(
        listEquals.equals(
            structA.dbSites.map((e) => e.copyWith(id: 0)).toList(),
            combinedSites.map((e) => e.copyWith(id: 0)).toList()),
        true);
  });
  ...
  test('merge DBs, skip incoming projects', () async {
    ...
    // Site checks
    expect(listEquals.equals(structA.dbSites, structA.sites), true);
  });
  ...
  test('merge DBs, incoming project maps to current', () async {
    ...
    final combinedSites = structA.sites +
        structB.sites
            .map((site) =>
                site.copyWith(projectUuid: Value(structA.projects.first.uuid)))
            .toList();
